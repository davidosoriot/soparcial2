...
### 5. Categoría: Editor (`editor`)

Categoría añadida por el equipo de 3 para integrar el editor de texto CLI del
parcial (`edi`). A diferencia de las categorías anteriores (demos puntuales
de una syscall que retornan de inmediato), este comando lanza una aplicación
interactiva completa como proceso hijo aislado.

* `editor [archivo]`: Lanza el editor `edi` (ver `../editor/`) vía `fork()` +
  `execve()`, esperando su finalización con `waitpid()`. El editor
  implementa `o p a d` (base) + `i s` (Nivel 2) + `m y x` (Nivel 3) usando
  únicamente `open/read/write/lseek/ftruncate/close/fstat`.
  * *Syscalls*: `readlink(2)`, `fork(2)`, `execve(2)`, `waitpid(2)`

La justificación completa de por qué se creó una categoría nueva en vez de
reutilizar `datos` está en el comentario de cabecera de `cat_editor.c` y en
el documento de sustentación (PDF) del proyecto.
