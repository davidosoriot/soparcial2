...
Command commands[] = {
    ...
    /* --- Categoría: Editor --- */
    {
        "editor", "editor",
        "editor [archivo]",
        "Lanza 'edi', el editor de texto interactivo del equipo (aplicacion completa, no una demo puntual de syscall), como proceso hijo aislado.",
        "readlink(2), fork(2), execve(2), waitpid(2)",
        cmd_editor
    }
};
...
void print_help(const char *arg) {
    if (arg == NULL) {
        ...
        printf("  " COLOR_CATEGORY "editor" COLOR_RESET "     - Editor de texto interactivo 'edi' (open, read, write, lseek, fstat, ...)\n\n");
        printf("Uso general:\n");
